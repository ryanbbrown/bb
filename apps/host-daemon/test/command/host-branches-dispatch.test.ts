...
describe("host.list_worktrees dispatch", () => {
  it("lists user-managed worktrees with canonical paths and branches", async () => {
    const repoPath = await initBranchRepo();
    const parentPath = await makeTempDir("bb-host-worktrees-parent-");
    const worktreePath = path.join(parentPath, "personal");
    await runGitCommand(
      ["worktree", "add", "-b", "personal", worktreePath, "main"],
      { cwd: repoPath },
    );

    const result = await dispatchOnlineRpcCommand(
      { type: "host.list_worktrees", path: repoPath },
      createHarness().dispatchOptions(),
    );

    expect(result.worktrees).toEqual(
      expect.arrayContaining([
        { path: await fs.realpath(repoPath), branchName: "develop" },
        { path: await fs.realpath(worktreePath), branchName: "personal" },
      ]),
    );
  });
});

describe("host.resolve_paths dispatch", () => {
  it("resolves aliases and reports missing paths", async () => {
    const targetPath = await makeTempDir("bb-host-resolve-path-target-");
    const parentPath = await makeTempDir("bb-host-resolve-path-parent-");
    const aliasPath = path.join(parentPath, "alias");
    const missingPath = path.join(parentPath, "missing");
    await fs.symlink(targetPath, aliasPath);

    const result = await dispatchOnlineRpcCommand(
      { type: "host.resolve_paths", paths: [aliasPath, missingPath] },
      createHarness().dispatchOptions(),
    );

    expect(result).toEqual({
      paths: [
        { path: aliasPath, canonicalPath: await fs.realpath(targetPath) },
        { path: missingPath, canonicalPath: null },
      ],
    });
  });
});
