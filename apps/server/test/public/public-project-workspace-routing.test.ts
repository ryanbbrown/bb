...
describe("public project workspace routing", () => {
  it("merges discovered worktrees with project environments", async () => {
    await withTestHarness(async (harness) => {
      const { host, session } = seedHostSession(harness.deps, {
        id: "host-project-worktrees",
      });
      ...
      registerHostRpcResponder(harness, {
        ...
        handle: (request) => {
          if (request.command.type === "host.resolve_paths") {
            expect(request.command.paths).toEqual(["/repo-managed-alias"]);
            return {
              ok: true,
              result: {
                paths: [
          ...
          return {
            ...
            result: {
              worktrees: [
                { path: "/repo-managed", branchName: "bb/managed" },
                { path: "/repo-personal", branchName: "personal" },
              ],
            },
          };
        },
