...
import { useAppFonts } from "@/theme/useAppFonts"; // module keeps the splash up until fonts load
import { UiProvider } from "@/ui";

export default function RootLayout() {
  const { ready } = useAppFonts(); // keeps the splash until Inter/Fira Code load
...
`UiProvider` = `ThemeProvider` + `SheetProvider` (`BottomSheetModalProvider`)

- `Toaster`. Use the pieces separately if the layout needs a different order.
...
- `useTheme()` → `{ palette, mode, preference, tokens, radii, typography, fonts, setMode }`.
  ...
  - `ThemeProvider` accepts `storage` (a `ThemePreferenceStorage`) for tests.
...
| `Button`                         | `variant` default·secondary·outline·ghost·destructive·link; `size` sm·default·lg·icon; `icon` (IconName), `iconPosition`; `loading`; `pressed` (toggle); `haptic` (`true`/light/medium/heavy/selection); `onPress`; string or node children; `className`, `textClassName` | Heights 36/40/48, icon 40×40.                                                                                                                                                                                                                          |
