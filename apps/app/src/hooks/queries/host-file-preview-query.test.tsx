...
describe("useHostFilePreview", () => {
  ...
  // A host path sits outside every workspace and thread-storage watch, so an
  // agent writing the file emits no event the app can see. These cover the
  // poll that keeps an open preview tracking the file.
  describe("live refresh", () => {
    const NOTES_PATH = "/tmp/notes.md";

    function mockTextRead(content: string) {
      filesSdk.read.mockResolvedValue({
        path: NOTES_PATH,
        content,
        contentEncoding: "utf8",
    ...
    function mockLease() {
      filesSdk.createPreview.mockResolvedValue({
        baseUrl: "/api/v1/file-previews/lease-live",
    ...
    it("picks up an out-of-band write while the preview stays open", async () => {
      ...
      const { wrapper } = createQueryClientTestHarness();
      const { result } = renderHook(
        () => useHostFilePreview("host-1", NOTES_PATH),
        { wrapper },
      );
      await waitFor(() => expect(result.current.isSuccess).toBe(true));
      expect(result.current.data).toMatchObject({ content: "# v1" });
      ...
      await waitFor(() =>
        expect(result.current.data).toMatchObject({ content: "# v2" }),
      );
    });

    it("refetches on visibility regain without waiting out a stale window", async () => {
      vi.useFakeTimers({ shouldAdvanceTime: true });
      mockLease();
      mockTextRead("# v1");
      const { wrapper } = createQueryClientTestHarness();
      ...
      // Only 2.5s elapsed, so the first poll is not due yet: the refetch came
      // from focus, which a long stale window would have suppressed. The
      // preceding case already covers the refetched content reaching the view.
      expect(filesSdk.read).toHaveBeenCalledTimes(2);
    });

    it("does not poll media, which resolves to a stable lease URL", async () => {
      vi.useFakeTimers({ shouldAdvanceTime: true });
      mockLease();
      const { wrapper } = createQueryClientTestHarness();
      const { result } = renderHook(
    ...
    it("stops polling once the panel closes", async () => {
      ...
      const { rerender, result } = renderHook(
        ({ enabled }) => useHostFilePreview("host-1", NOTES_PATH, { enabled }),
        { initialProps: { enabled: true }, wrapper },
      );
      await waitFor(() => expect(result.current.isSuccess).toBe(true));

      rerender({ enabled: false });
      const readsAtClose = filesSdk.read.mock.calls.length;
      await act(async () => {
        await vi.advanceTimersByTimeAsync(30_000);
      });

      expect(filesSdk.read).toHaveBeenCalledTimes(readsAtClose);
    });
  });
});
