...
- apps/app/src/views/SettingsView.tsx + components/settings/*: **headless-logic-only** — Radix DropdownMenu/Switch/Tooltip, Tailwind, `document`/CSS mask for favicon preview (SettingsView.tsx:267-279), navigator.mediaDevices for voice, Electron desktop update API in UpdatesSettingsSection. Section list (settings-nav.tsx SETTINGS_NAV_SECTIONS), mutations (settings-mutations), and preference hooks are portable except those on localStorage (`createLocalStorageSyncStorage`).
