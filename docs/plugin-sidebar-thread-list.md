...
Region and thread order are authoritative. Sticky regions must come first and BB measures their native stack so later flow labels stay below it. `flat` ignores parent relationships; `native` applies BB's shared parent-tree, cross-project, and native worktree rules after the supplied source order. Project grouping uses each thread's own project in `flat` mode and the native root project in `native` mode. Project groups and rows use BB's windowing and placeholder navigation. A headerless region cannot be collapsible. Project and region collapse controls exist only when their `collapsible` field is true.

A complete projection must account for every eligible thread exactly once, either in one region or in `excludedThreadIds`. Eligibility means `thread.visibility === "visible" && !thread.isArchived`; hidden, archived, and deleted IDs must be omitted, not excluded. For example, if the hook returns one visible thread and one hidden side chat, put only the visible ID in a region or `excludedThreadIds` and do not submit the hidden ID anywhere.

BB rejects the complete projection for malformed fields, blank or duplicate region IDs, headerless collapse, sticky regions after flow regions, duplicate, unknown, stale, archived, hidden, deleted, missing, or visible-and-excluded threads, and unknown, duplicate, or omitted represented projects. Limits are 64 regions, 50,000 combined visible plus excluded thread references, 10,000 combined project references, 256 characters per submitted string, and 320 characters per diagnostic. Raw array lengths are checked before their elements are read. Rejection renders `experimental_Original` atomically, logs one bounded reason, and shows one toast for each plugin generation and failure reason. A later valid projection is retried normally.
